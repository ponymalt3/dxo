...
public:
  ...
  AlsaPluginDxO(const std::string& path, uint32_t blockSize, uint32_t periodSize)
      : blockSize_(blockSize),
        ...
        outputBuffer_{new int16_t[blockSize_ * 100 * kNumOutputChannels]},
        thread_([this]() { run(); })
  {
    ...
    while(threadRun_)
    {
      ...
      while(i < size)
      {
        ...
        if(inputOffset_ == blockSize_)
        {
          ...
          std::cout << "time " << time_taken << " ns" << std::endl;
          ...
          if(dstSize++ == 10)
          {
            ...
            result = snd_pcm_writei(pcm_, outputBuffer_.get(), blockSize_ * 10);
            {
              if(result == -EAGAIN)
              {
