...
public:
  ...
  uint32_t update(PcmStream<_InputSampleType>& src, uint32_t size)
  {
    ...
    PcmStream<int16_t> dst{outputBuffers_.getWriteBuffer(), kNumOutputChannels};
    ...
    while(i < size)
    {
      ...
      if(inputOffset_ == blockSize_)
      {
        ...
        inputOffset_ = 0;
        dst = PcmStream<int16_t>{outputBuffers_.getWriteBuffer(), kNumOutputChannels};
      }
    }
  ...
  void syncOutputBuffer()
  {
    while(threadRun_)
    {
      auto result = snd_pcm_writei(pcm_, outputBuffers_.getReadBuffer(), blockSize_);
