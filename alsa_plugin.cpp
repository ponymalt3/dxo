
#include "alsa_plugin.h"

#include <alsa/asoundlib.h>
#include <alsa/pcm_external.h>
#include <poll.h>
#include <sys/eventfd.h>
#include <unistd.h>

AlsaPluginDxO::AlsaPluginDxO(uint32_t blockSize,
                             uint32_t firDelay,
                             const std::string slavePcm,
                             const snd_pcm_ioplug_callback_t* callbacks)
    : blockSize_(blockSize),
      firDelay_(firDelay),
      inputs_(3),
      outputs_(7),
      inputOffset_(0),
      outputBuffer_{new int16_t[blockSize_ * kNumOutputChannels]},
      pcmName_(slavePcm)
{
  memset(this, 0, sizeof(snd_pcm_ioplug_t));

  // Init ALSA structure
  snd_pcm_ioplug_t::version = SND_PCM_IOPLUG_VERSION;
  snd_pcm_ioplug_t::name = "dxo";
  snd_pcm_ioplug_t::private_data = this;
  snd_pcm_ioplug_t::callback = callbacks;

  pollFd_ = eventfd(1, EFD_CLOEXEC | EFD_NONBLOCK);
  snd_pcm_ioplug_t::poll_fd = pollFd_;
  snd_pcm_ioplug_t::poll_events = POLLIN;

  enableLogging();
}

bool AlsaPluginDxO::initialize(const std::string& path, bool normalize)
{
  auto coeffs = loadFIRCoeffs(path, kScaleS16LE, normalize);

  if(coeffs.size() != kNumFilters)
  {
    print("expected ", static_cast<uint32_t>(kNumFilters), " filters in '", path, "', got ", coeffs.size());
    return false;
  }

  std::vector<FirMultiChannelCrossover::ConfigType> config{{0, coeffs[0]},
                                                           {0, coeffs[1]},
                                                           {0, coeffs[2]},
                                                           {1, coeffs[3]},
                                                           {1, coeffs[4]},
                                                           {1, coeffs[5]},
                                                           {2, coeffs[6]}};

  crossover_ = std::make_unique<FirMultiChannelCrossover>(blockSize_, 3, config, 3);

  for(auto i{0}; i < inputs_.size(); ++i)
  {
    inputs_[i] = crossover_->getInputBuffer(i).data();
  }

  for(auto i{0}; i < outputs_.size(); ++i)
  {
    outputs_[i] = crossover_->getOutputBuffer(i).data();
  }

  return true;
}

std::vector<std::vector<float>> AlsaPluginDxO::loadFIRCoeffs(const std::string& path,
                                                             float outputFormatScaling,
                                                             bool normalize)
{
  std::ifstream file(path);

  if(!file)
  {
    return std::vector<std::vector<float>>{};
  }

  std::vector<std::vector<float>> filters;
  while(!file.eof())
  {
    std::string line;
    while(std::getline(file, line))
    {
      uint32_t posNotWhiteSpace = 0;
      while(std::isspace(line[posNotWhiteSpace]))
      {
        ++posNotWhiteSpace;
      }

      if(line.substr(posNotWhiteSpace).starts_with('#') || line.length() == posNotWhiteSpace)
      {
        continue;
      }

      std::vector<float> coeffs;
      std::istringstream iss(line);
      while(!iss.eof())
      {
        double value = 0;
        iss >> value;
        coeffs.push_back(static_cast<float>(value));
      }

      if(coeffs.size() > 0)
      {
        filters.push_back(coeffs);
      }
    }
  }

  float maxL1 = 0.0f;
  for(const auto& filter : filters)
  {
    float l1 = 0.0f;
    for(auto coeff : filter)
    {
      l1 += std::abs(coeff);
    }

    maxL1 = std::max(maxL1, l1);
  }

  float gain = outputFormatScaling;

  // need to reduce gain to prevent clipping of output sample
  if(normalize && maxL1 > 1.0f)
  {
    gain /= maxL1;

    print(std::format("normalizing all filters by {:.4f} ({:.2f} dB), worst sum|h| was {:.4f}",
                      1.0f / maxL1,
                      20.0 * std::log10(1.0f / maxL1),
                      maxL1));
  }

  for(auto& filter : filters)
  {
    for(auto& coeff : filter)
    {
      coeff *= gain;
    }
  }

  return filters;
}

void AlsaPluginDxO::enableLogging()
{
  logging_.open("/tmp/dxo.txt", std::ios::app | std::ios::out);
}

bool AlsaPluginDxO::writePcm(const int16_t* data, const uint32_t frames)
{
  if(!pcm_output_device_)
  {
    return false;
  }

  uint32_t written = 0;
  while(written < frames)
  {
    const auto result =
        snd_pcm_writei(pcm_output_device_, data + written * kNumOutputChannels, frames - written);

    if(result < 0)
    {
      print("write error [", snd_strerror(result), "]");
      const auto recovered = snd_pcm_recover(pcm_output_device_, result, 1);
      if(recovered < 0)
      {
        print("snd_pcm_recover failed [", snd_strerror(recovered), "]");
      }
      return false;
    }

    if(result == 0)
    {
      return false;
    }

    written += result;
  }

  return true;
}

int AlsaPluginDxO::openPcm()
{
  if(pcm_output_device_)
  {
    return 0;
  }

  const auto result = snd_pcm_open(&(pcm_output_device_), pcmName_.c_str(), SND_PCM_STREAM_PLAYBACK, 0);

  if(result < 0)
  {
    pcm_output_device_ = nullptr;
    print("snd_pcm_open failed ", snd_strerror(result));
    return -EBUSY;
  }

  snd_pcm_hw_params_t* params{nullptr};
  snd_pcm_hw_params_alloca(&params);
  memset(params, 0, snd_pcm_hw_params_sizeof());

  snd_pcm_hw_params_any(pcm_output_device_, params);
  // snd_pcm_hw_params_dump(params, output_);

  if(!configureHwParams(params))
  {
    return closePcm(-EINVAL);
  }

  if(!verifyParams(params))
  {
    print(std::format("slave did not accept {} channels", static_cast<uint32_t>(kNumOutputChannels)));
    return closePcm(-EINVAL);
  }

  configureTiming(params);

  updateChannelMap();

  return 0;
}

int AlsaPluginDxO::closePcm(int error)
{
  if(pcm_output_device_)
  {
    snd_pcm_close(pcm_output_device_);
    pcm_output_device_ = nullptr;
  }

  return error;
}

bool AlsaPluginDxO::configureHwParams(snd_pcm_hw_params_t* params)
{
  if(snd_pcm_hw_params_set_access(pcm_output_device_, params, SND_PCM_ACCESS_RW_INTERLEAVED) < 0)
  {
    print("snd_pcm_hw_params_set_access failed");
    return false;
  }

  if(snd_pcm_hw_params_set_format(pcm_output_device_, params, SND_PCM_FORMAT_S16_LE) < 0)
  {
    print("snd_pcm_hw_params_set_format failed");
    return false;
  }

  if(snd_pcm_hw_params_set_channels(pcm_output_device_, params, kNumOutputChannels) < 0)
  {
    print("snd_pcm_hw_params_set_channels failed");
    return false;
  }

  uint32_t slaveRate = rate;
  if(snd_pcm_hw_params_set_rate_near(pcm_output_device_, params, &slaveRate, 0) < 0)
  {
    print("snd_pcm_hw_params_set_rate_near failed");
  }

  snd_pcm_uframes_t periodSize = blockSize_;
  if(snd_pcm_hw_params_set_period_size_near(pcm_output_device_, params, &periodSize, nullptr) < 0)
  {
    print("snd_pcm_hw_params_set_period_size_near failed");
  }

  // if slave buffer size is specified use it otherwise take the buffer size of plugin
  snd_pcm_uframes_t bufferSize = buffer_size;

  if(snd_pcm_hw_params_set_buffer_size_near(pcm_output_device_, params, &bufferSize) < 0)
  {
    print("snd_pcm_hw_params_set_buffer_size_near failed");
  }

  if(snd_pcm_hw_params(pcm_output_device_, params) < 0)
  {
    print("snd_pcm_hw_params failed");
    return false;
  }

  return true;
}

bool AlsaPluginDxO::configureTiming(snd_pcm_hw_params_t* params)
{
  snd_pcm_uframes_t period{0};
  snd_pcm_uframes_t buffer{0};
  snd_pcm_hw_params_get_period_size(params, &period, nullptr);
  snd_pcm_hw_params_get_buffer_size(params, &buffer);

  snd_pcm_sw_params_t* swParams{nullptr};
  snd_pcm_sw_params_alloca(&swParams);

  if(snd_pcm_sw_params_current(pcm_output_device_, swParams) >= 0)
  {
    snd_pcm_sw_params_set_start_threshold(pcm_output_device_, swParams, buffer);
    snd_pcm_sw_params_set_avail_min(pcm_output_device_, swParams, period);

    if(snd_pcm_sw_params(pcm_output_device_, swParams) < 0)
    {
      print("snd_pcm_sw_params failed");
      return false;
    }
  }

  return true;
}

bool AlsaPluginDxO::verifyParams(snd_pcm_hw_params_t* params)
{
  uint32_t slaveRate{0};
  uint32_t slaveChannels{0};
  snd_pcm_uframes_t slavePeriod{0};
  snd_pcm_uframes_t slaveBufferSize{0};

  snd_pcm_hw_params_get_rate(params, &slaveRate, nullptr);
  snd_pcm_hw_params_get_channels(params, &slaveChannels);
  snd_pcm_hw_params_get_period_size(params, &slavePeriod, nullptr);
  snd_pcm_hw_params_get_buffer_size(params, &slaveBufferSize);

  print(std::format("slave '{}': rate={} channels={} period={} buffer={}",
                    pcmName_,
                    slaveRate,
                    slaveChannels,
                    slavePeriod,
                    slaveBufferSize));

  return slaveChannels == kNumOutputChannels;
}

bool AlsaPluginDxO::updateChannelMap()
{
  auto chMap = snd_pcm_get_chmap(pcm_output_device_);

  if(chMap == nullptr)
  {
    return false;
  }

  const auto numChannels = std::min<uint32_t>(chMap->channels, channelMap_.size());

  for(auto i{0U}; i < numChannels; ++i)
  {
    const auto pos = chMap->pos[i];
    channelMap_[i] =
        (pos < kMapAlsaChannel.size()) ? kMapAlsaChannel[pos] : static_cast<uint32_t>(kChUnknown);
    print("CHMAP[", i, "]: ", pos, "  -> ", channelMap_[i]);
  }

  free(chMap);

  return true;
}

extern "C" {

// Channel map (ALSA channel definitions)
struct ChannelMap
{
  uint32_t num_channels;
  std::array<unsigned int, 3> channels{};
};

static const ChannelMap kChannelMaps[] = {{2, {SND_CHMAP_FL, SND_CHMAP_FR, SND_CHMAP_UNKNOWN}},
                                          {3, {SND_CHMAP_FL, SND_CHMAP_FR, SND_CHMAP_LFE}}};
const int32_t kNumChannelMaps = std::size(kChannelMaps);

snd_pcm_sframes_t AlsaPluginDxO::dxo_pointer(snd_pcm_ioplug_t* io)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);
  return plugin->streamPos_ % plugin->buffer_size;
}

snd_pcm_sframes_t AlsaPluginDxO::dxo_transfer(snd_pcm_ioplug_t* io,
                                              const snd_pcm_channel_area_t* src_areas,
                                              snd_pcm_uframes_t src_offset,
                                              snd_pcm_uframes_t size)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);

  const auto writer = [plugin](const int16_t* data, uint32_t frames) {
    return plugin->writePcm(data, frames);
  };

  if(io->format == SND_PCM_FORMAT_S16_LE)
  {
    PcmStream<int16_t> src(src_areas, src_offset);
    plugin->update(src, size, io->channels == 3, writer);
  }
  else if(io->format == SND_PCM_FORMAT_FLOAT_LE)
  {
    PcmStream<float> src(src_areas, src_offset);
    plugin->update(src, size, io->channels == 3, writer);
  }

  return size;
}

int AlsaPluginDxO::dxo_prepare(snd_pcm_ioplug_t* io)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);

  plugin->print("dxo_prepare");
  plugin->streamPos_ = 0;
  plugin->inputOffset_ = 0;

  if(plugin->pcm_output_device_)
  {
    const auto state = snd_pcm_state(plugin->pcm_output_device_);

    if(state == SND_PCM_STATE_XRUN || state == SND_PCM_STATE_SUSPENDED)
    {
      const auto result = snd_pcm_prepare(plugin->pcm_output_device_);
      if(result < 0)
      {
        plugin->print("slave prepare failed [", snd_strerror(result), "]");
      }
    }
  }

  return 0;
}

int AlsaPluginDxO::dxo_close(snd_pcm_ioplug_t* io)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);

  plugin->print("dxo_close");
  plugin->print("avg time: ",
                plugin->totalBlocks_ > 0 ? plugin->totalTime_ / plugin->totalBlocks_ : 0.0,
                " over ",
                plugin->totalBlocks_,
                " blocks");

  if(plugin->pcm_output_device_)
  {
    snd_pcm_drain(plugin->pcm_output_device_);
    plugin->closePcm(0);
  }

  if(plugin->pollFd_ >= 0)
  {
    close(plugin->pollFd_);
    plugin->pollFd_ = -1;
  }

  delete plugin;

  return 0;
}

snd_pcm_chmap_query_t** AlsaPluginDxO::dxo_query_chmaps(snd_pcm_ioplug_t* io)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);
  plugin->print("dxo_query_chmaps");

  auto maps =
      static_cast<snd_pcm_chmap_query_t**>(malloc(sizeof(snd_pcm_chmap_query_t*) * (kNumChannelMaps + 1)));

  if(!maps)
  {
    plugin->print("  invalid maps");
    return nullptr;
  }

  for(auto i{0}; i < kNumChannelMaps; ++i)
  {
    maps[i] = static_cast<snd_pcm_chmap_query_t*>(
        malloc(sizeof(snd_pcm_chmap_query_t) + sizeof(kChannelMaps[i].channels)));

    if(maps[i] == nullptr)
    {
      plugin->print("  invalid maps[i]");
      snd_pcm_free_chmaps(maps);
      return nullptr;
    }

    maps[i]->type = SND_CHMAP_TYPE_FIXED;
    maps[i]->map.channels = kChannelMaps[i].num_channels;
    memcpy(maps[i]->map.pos, kChannelMaps[i].channels.data(), sizeof(kChannelMaps[i].channels));
  }

  maps[kNumChannelMaps] = nullptr;

  return maps;
}

snd_pcm_chmap_t* AlsaPluginDxO::dxo_get_chmap(snd_pcm_ioplug_t* io)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);

  auto map =
      static_cast<snd_pcm_chmap_t*>(malloc(sizeof(snd_pcm_chmap_t) + sizeof(kChannelMaps[0].channels)));

  if(map)
  {
    const auto map_index =
        std::min(std::max(static_cast<int32_t>(plugin->channels) - 2, 0), kNumChannelMaps - 1);
    map->channels = plugin->channels;
    memcpy(map->pos, kChannelMaps[map_index].channels.data(), sizeof(kChannelMaps[0].channels));
  }

  return map;
}

int AlsaPluginDxO::dxo_hw_params(snd_pcm_ioplug_t* io, snd_pcm_hw_params_t* params)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);
  plugin->print("dxo_hw_params");

  snd_pcm_hw_params_get_rate(params, &plugin->rate, 0);
  snd_pcm_hw_params_get_channels(params, &plugin->channels);
  snd_pcm_hw_params_get_format(params, &plugin->format);
  snd_pcm_hw_params_get_period_size(params, &plugin->period_size, 0);
  snd_pcm_hw_params_get_buffer_size(params, &plugin->buffer_size);
  snd_pcm_hw_params_get_access(params, &plugin->access);

  return plugin->openPcm();
}

int AlsaPluginDxO::dxo_delay(snd_pcm_ioplug_t* io, snd_pcm_sframes_t* delayp)
{
  auto* plugin = reinterpret_cast<AlsaPluginDxO*>(io);

  const auto convDelay = plugin->blockSize_ - plugin->inputOffset_;

  if(!plugin->pcm_output_device_)
  {
    *delayp = plugin->firDelay_ + convDelay;
    return 0;
  }

  snd_pcm_sframes_t slaveDelay{0};
  const auto result = snd_pcm_delay(plugin->pcm_output_device_, &slaveDelay);
  if(result < 0)
  {
    plugin->print("snd_pcm_delay failed [", snd_strerror(result), "]");
    return result;
  }

  *delayp = slaveDelay + plugin->firDelay_ + convDelay;
  return 0;
}

static const snd_pcm_ioplug_callback_t callbacks = {
    .start = [](snd_pcm_ioplug_t*) { return 0; },
    .stop = [](snd_pcm_ioplug_t*) { return 0; },
    .pointer = &AlsaPluginDxO::dxo_pointer,
    .transfer = &AlsaPluginDxO::dxo_transfer,
    .close = &AlsaPluginDxO::dxo_close,
    .hw_params = &AlsaPluginDxO::dxo_hw_params,
    .prepare = &AlsaPluginDxO::dxo_prepare,
#if SND_PCM_EXTPLUG_VERSION >= 0x10002
    .delay = &AlsaPluginDxO::dxo_delay,
    .query_chmaps = &AlsaPluginDxO::dxo_query_chmaps,
    .get_chmap = &AlsaPluginDxO::dxo_get_chmap,
#endif
};

SND_PCM_PLUGIN_DEFINE_FUNC(dxo)
{
  long int blockSize = 128;
  long int firDelay = 0;  // ignore fir delay by default
  long int normalize = 1;
  std::string coeffPath;
  std::string slavePcm;
  snd_config_t* slaveConfig = nullptr;

  snd_config_iterator_t i, next;
  snd_config_for_each(i, next, conf)
  {
    snd_config_t* config = snd_config_iterator_entry(i);
    const char* id;
    snd_config_get_id(config, &id);

    std::string param(id);

    if(param == "slave")
    {
      snd_config_iterator_t i, next;
      snd_config_for_each(i, next, config)
      {
        snd_config_t* config = snd_config_iterator_entry(i);
        const char* id;
        snd_config_get_id(config, &id);

        if(std::string(id) == "pcm")
        {
          const char* str;
          snd_config_get_string(config, &str);
          slavePcm = str;
          break;
        }
      }
      continue;
    }

    if(param == "blocksize")
    {
      snd_config_get_integer(config, &blockSize);
      continue;
    }

    if(param == "fir_delay")
    {
      snd_config_get_integer(config, &firDelay);
      firDelay = std::max(0L, firDelay);
      continue;
    }

    if(param == "normalize")
    {
      snd_config_get_integer(config, &normalize);
      continue;
    }

    if(param == "path")
    {
      const char* path;
      snd_config_get_string(config, &path);
      coeffPath = path;
      continue;
    }
  }

  if(slavePcm.length() == 0)
  {
    return -EINVAL;
  }

  auto* plugin = new AlsaPluginDxO(blockSize, firDelay, slavePcm, &callbacks);

  const auto error = [&](const std::string& reason, int error = -EINVAL) {
    SNDERR("%s", reason.c_str());
    plugin->print(reason);

    if(plugin->pcm != nullptr)
    {
      snd_pcm_ioplug_delete(plugin);
    }
    else
    {
      delete plugin;
    }

    return error;
  };

  if(!std::ifstream(coeffPath))
  {
    return error("DxO: Cant open coefficients file '" + coeffPath + "'");
  }

  if(!plugin->initialize(coeffPath, normalize != 0))
  {
    return error("DxO: Cant initialize DxO plugin");
  }

  auto result = snd_pcm_ioplug_create(plugin, name, stream, mode);

  if(result < 0)
  {
    return error("DxO: snd_pcm_ioplug_create failed", result);
  }

  static constexpr uint32_t supportedAccess[] = {SND_PCM_ACCESS_RW_INTERLEAVED};
  static constexpr uint32_t supportedFormats[] = {SND_PCM_FORMAT_S16_LE, SND_PCM_FORMAT_FLOAT_LE};
  static constexpr uint32_t supportedHwRates[] = {44100, 48000};

  if(snd_pcm_ioplug_set_param_list(
         plugin, SND_PCM_IOPLUG_HW_ACCESS, std::size(supportedAccess), supportedAccess) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_ACCESS failed");
  }

  if(snd_pcm_ioplug_set_param_list(
         plugin, SND_PCM_IOPLUG_HW_FORMAT, std::size(supportedFormats), supportedFormats) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_FORMAT failed");
  }

  if(snd_pcm_ioplug_set_param_minmax(plugin, SND_PCM_IOPLUG_HW_CHANNELS, 2, 3) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_CHANNELS failed");
  }

  if(snd_pcm_ioplug_set_param_minmax(plugin, SND_PCM_IOPLUG_HW_PERIOD_BYTES, 16, 2 * 1024 * 1024) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_PERIOD_BYTES failed");
  }

  if(snd_pcm_ioplug_set_param_minmax(plugin, SND_PCM_IOPLUG_HW_BUFFER_BYTES, 16, 2 * 1024 * 1024) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_BUFFER_BYTES failed");
  }

  if(snd_pcm_ioplug_set_param_list(
         plugin, SND_PCM_IOPLUG_HW_RATE, std::size(supportedHwRates), supportedHwRates) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_RATE failed");
  }

  if(snd_pcm_ioplug_set_param_minmax(plugin, SND_PCM_IOPLUG_HW_PERIODS, 1, 1024) < 0)
  {
    return error("DxO: SND_PCM_IOPLUG_HW_PERIODS failed");
  }

  *pcmp = plugin->pcm;

  plugin->print("DxO: Initialization complete");

  return 0;
}

SND_PCM_PLUGIN_SYMBOL(dxo);
}
