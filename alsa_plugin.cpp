...
int dxo_prepare(snd_pcm_ioplug_t* ext)
{
  ...
  if(snd_pcm_hw_params_set_period_size(plugin->pcm_, plugin->params_, plugin->blockSize_ / 2, 0) < 0)
  {
    snd_output_printf(plugin->output_, "Can't set period size.");
  }

  if(snd_pcm_hw_params_set_buffer_size(plugin->pcm_, plugin->params_, plugin->blockSize_ * 512) < 0)
  {
    snd_output_printf(plugin->output_, "Can't set period size.");
  }

  if(snd_pcm_hw_params_set_periods(plugin->pcm_, plugin->params_, 1024, 0) < 0)
  {
    snd_output_printf(plugin->output_, "Can't set period size.");
  }
